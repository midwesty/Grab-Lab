window.GrabLabModal = (() => {
  ...
  function openModal(modalId, options = {}) {
    ...
    const el = modal.el;
    const wasOpen = Boolean(modal.isOpen);
    const forceCenter = options.forceCenter === true || options.resetPosition === true;

    modal.isOpen = true;
    U.show(el, "block");

    /*
      Important MVP stability rule:
      Background systems redraw the HUD/world every few seconds. Some buttons or
      event flows can safely call openModal() again while a modal is already open.
      Re-centering an already-open modal makes menus appear to jump, reset, or
      shuffle around. Existing open modals should stay exactly where the player
      left them unless the caller explicitly asks for a reset/force center.
    */
    if (wasOpen && !forceCenter) {
      clampModalToViewport(el);
    } else if (forceCenter || modal.lastLeft == null || modal.lastTop == null || options.center !== false) {
      centerModal(el);
      requestAnimationFrame(() => {
        clampModalToViewport(el);
        saveModalPosition(el);
      });
    } else {
    ...
    if (!wasOpen && options.playSound !== false) {
      A.playSfx("ui_confirm").catch?.(() => {});
    }

    U.eventBus.emit(wasOpen ? "ui:modalRefocused" : "ui:modalOpened", modal.id);
    return true;
  }
